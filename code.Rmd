...
Here we want to check if there is any effect of planting date on the risk of not-offsetting the cost.
...
prof1 %>% 
  ...
  labs(x = "Risk of not offsetting cost",
       fill = "Risk", 
       title = "All locations")+
  facet_grid(treatment ~ planting_order)
```

##### Piracicaba

Now we plot only the locations where five plantings were tested. First, Piracicaba.

```{r fig.height=5, fig.width=10}
prof1 %>% 
  filter(trial_code == "PIR") %>% 
  ggplot(aes(x= ploss, fill = ploss))+
  geom_histogram(aes(fill = ..x..), color = "white", bins = 20 ) + 
       scale_fill_gradient(low='green',  high='darkred')+
     geom_vline(xintercept =  0.5, linetype = 2)+
...
### Overall risk of not-offsetting costs
...
#### Number of sprays and risk


Let's see the relationship between the number of sprays and the risk of not-ofsetting the costs.
...
#### Number of sprays and yield response
